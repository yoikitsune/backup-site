...
**Objectif** : Sauvegarder un site WordPress en production (o2switch) et le restaurer localement dans Docker pour vérifier que tout fonctionne.
...
```
PRODUCTION (o2switch)          LOCAL (ton ordinateur)
┌─────────────────────┐        ┌──────────────────────────────┐
...
- [x] Versions correspondent à o2switch
...
## 💾 Étape 3 : Sauvegarder depuis o2switch
...
2. **Exécution de mysqldump** sur le serveur
   ```bash
   mysqldump -h localhost -u [REDACTED] -p'[REDACTED]' \
     --routines --triggers --events \
     [REDACTED] | gzip > /tmp/database.sql.gz
   ```
...
**Résultat attendu** :
```
-rw-rw-r-- 1 julien julien [REDACTED] Nov  9 12:43 database.sql.gz
-rw-rw-r-- 1 julien julien [REDACTED] Nov  9 12:43 files.tar.gz
```

### Checklist Étape 3

- [x] Sauvegarde des fichiers réussie
- [x] Sauvegarde de la BDD réussie
...
| 3 | Sauvegarder o2switch | 1 min | 78 MB sauvegardés ✅ |
...
- ✅ Sauvegardes créées depuis o2switch
...
- Les versions Docker correspondent exactement à o2switch
